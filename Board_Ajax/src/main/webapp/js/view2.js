...
function updateForm(num){ //num : 수정할 댓글 글번호
	...
	//수정 폼이 이는 상태에서 더보기를 클릭할 수 없도록 더 보기 영역을 숨겨요
	$(".commnet-tool").hide();
	...
	//$('.comment-list_.comment-write').clone() : 기본 글쓰기 영역 복사합니다.
...
function del(num){//num : 댓글 번호
  	if(!confirm("정말 삭제하시겠습니까?")){
		  $("#comment-list-item-layer" + num).hide();//'수정 삭제' 영역 숨겨주세요.
		  return;
	  }
	  $.ajax({
...
function replyform(num,lev,seq,ref){
	...
	$num_next.find('textarea').attr('placeholder','답글을 남겨주세요.');
...
$(function() {
	...
	$('.comment-area').on('click','.btn-cancel',function(){
		...
		//수정 폼이 있는 상태에서 더보기를 클릭한 수 없도록 더 보기 영역을 숨겼는데 취소를 선택하면 보여주도록 합니다.
		$(".comment-tool").show();
	})//수정 후 취소 버튼을 클릭한 경우
